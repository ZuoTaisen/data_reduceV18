...
class data_info():
    def __init__(self):
        ...
        self.D3ToMod = 33500
        self.TimeDelay = 52.7 #17.628 #info.instrument_info.TimeDelayD3 #17.628#52.7     #18.628  #ms  6埃，2.2埃和4埃的延时分别为：52.7ms，19.32ms和35.135ms
        self.TOF = 40              #ms
        #self.TofBins = 100
        self.QMin = 0.002        #A^-1
...
#WavePlot = np.sum(np.sum(DataStacked,axis = 0),axis = 0)
plt.plot(WavelengthArray,output)
plt.show()
with open(PlusMinus + '_Transmission.dat' + PlusPlus,'w') as f:
    for i in range(len(output)):
        print(WavelengthArray[i],'  ',output[i],file = f)

'''

#DataD3Bkg,tofBkg = load_data2(RunNumBkg)
#D3Mask = np.load('../masks/D3Mask.npy')
Data3D = DataD3 # - DataD3Bkg #3D*D3Mask[:,:,None]
plot_data(DataD3,'D3data')
...
LongPixels = 250
TofBins = 200
D3Center = [Info.XCenter+Info.BankWidth/2,Info.YCenter+Info.BankHeight/2]


D3X = np.linspace(0,Info.BankWidth,int(Info.BankWidth/Info.TubeWidth))
D3Y = np.linspace(0,Info.BankHeight,int(Info.BankHeight/Info.TubeHeight))
...
plt.xlabel('QX')
plt.ylabel('QY')
plt.savefig('QPlotD3_2D.png',dpi = 600, format = 'png',bbox_inches = 'tight', transparent = True)
plt.show()


plt.figure(figsize=(5, 5))
plt.contour(QX,QY,QArray_2,80,cmap = 'hot')
plt.xlabel('QX (Angstrom$^{-1}$)')
plt.ylabel('QY (Angstrom$^{-1}$)')
plt.savefig('QPlot_2D_D3.png',dpi = 600, format = 'png',bbox_inches = 'tight', transparent = True)
plt.show()
