...
def time_diff(time1,time2):
    ...
    #print(time1,time2)
    # 将字节格式转换为字符串
    #byte_length1 = (time1.bit_length() + 7) // 8  # 计算所需的字节数
    #byte_length2 = (time2.bit_length() + 7) // 8  # 计算所需的字节数
    #time1 = time1.to_bytes(byte_length1, byteorder='big')
    #time2 = time2.to_bytes(byte_length2, byteorder='big')
...
def load_data2(RunNum):
    ...
    #startTime,endTime,useTimeSec,useTimeMin = time_func.get_experimental_time(DataFold,RunNum)
    startTime,endTime,useTimeSec,useTimeMin = time_func.get_experimental_time_info(DataFold,RunNum)
...
#    Data2Reshaped = np.reshape(data2,(64,250,5000))
    ...
    print('Used time is:',useTimeSec,'seconds or ',np.round(useTimeMin,2),'Minutes')   
    ...
    print('Count rate of  (n/s) \nD31: ' + str(round(Counts1/useTimeSec)) + '\nD32: ' + str(round(Counts2/useTimeSec)) )
